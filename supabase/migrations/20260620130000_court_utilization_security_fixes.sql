-- ============================================================
-- Court utilization — security & correctness fixes
-- Supersedes the function bodies from 20260620120000_court_utilization_rpcs.sql.
-- Self-contained & idempotent (CREATE OR REPLACE, no return-signature changes),
-- so it is safe to run whether or not the base migration was applied.
--
-- Fixes:
--   C1  revenue_cents returned only to admins (NULL for club managers)
--   H1  REVOKE EXECUTE from PUBLIC/anon on all four functions (least privilege)
--   H2  bound p_months to 1..24 (+ statement_timeout) to prevent DoS loops
--   H3  GREATEST(duration,0) guards negative-duration bookings
--   L4  helper restricted to owner-only (callers invoke it as SECURITY DEFINER owner)
--
-- Decisions baked in (see audit):
--   L1  Managers intentionally see their assigned courts even when the parent
--       location is offline (is_online filter applies to the ADMIN network view only).
-- ============================================================

-- ------------------------------------------------------------
-- Helper: total open minutes for a location across one month (unchanged body)
-- ------------------------------------------------------------
  ...
  WHILE v_day < v_month_end LOOP
    ...
    IF v_entry IS NOT NULL AND v_entry ? 'open' AND v_entry ? 'close'
      ...
      ELSIF v_close_min < v_open_min THEN
        v_total := v_total + ((1440 - v_open_min) + v_close_min);  -- crosses midnight
      END IF;
    END IF;
...
-- ------------------------------------------------------------
-- H1 / L4: least privilege. Revoke the Postgres default PUBLIC grant,
-- then re-grant EXECUTE only to authenticated for the three caller-facing RPCs.
-- The helper stays owner-only (the SECURITY DEFINER functions call it as owner).
-- ------------------------------------------------------------
REVOKE EXECUTE ON FUNCTION public.location_open_minutes(jsonb, boolean, date)   FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_court_utilization(date)                    FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_court_utilization_trend(uuid, int)         FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_network_utilization_trend(int)            FROM PUBLIC, anon, authenticated;
